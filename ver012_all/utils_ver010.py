...
def plot_tsne(X, label, prototype, label_text=None, save_path=None):
    ...
    sc.pp.neighbors(adata, n_neighbors=15) #used to be 30
    ...
    # sc.tl.tsne(adata)  #myles code
    print('umap')
    sc.tl.umap(adata, min_dist=0.01)  #myles code
    print("UMAP Complete: {:d} min {:d} s".format(int((time.time()-start) // 60), int((time.time()-start) % 60)))

    print('printing len set label')
    print( len(set(label)) )
    ...
    if label_text is None:
        ...
        #myles added
        p = sns.scatterplot(x=adata.obsm["X_umap"][:,0][:-n_proto], y=adata.obsm["X_umap"][:,1][:-n_proto], hue=label, s=5, palette=pal, alpha=0.8)
    else:
        #original
        ...
        #myles added
        p = sns.scatterplot(x=adata.obsm["X_umap"][:,0][:-n_proto], y=adata.obsm["X_umap"][:,1][:-n_proto], hue=[label_text[i] for i in label], s=5, palette=pal, alpha=0.8)
    ...
    for i in range(n_proto):
        ...
        #myles
        p.scatter(x=adata.obsm["X_umap"][i-n_proto,0], y=adata.obsm["X_umap"][i-n_proto,1], marker="p", s=150, color=pal_proto[i], label=label_proto[i])
...
def plot_tsne_w_cont(X, label, prototype, contribution, class_text, label_text=None, save_path=None, s=5, pattern = "tab10"):
    ...
    if label_text is None:
        ...
        p = sns.scatterplot(x=adata.obsm["X_umap"][:,0][:-n_proto-n_class*n_sample], y=adata.obsm["X_umap"][:,1][:-n_proto-n_class*n_sample], hue=label, s=s, palette=pal, alpha=0.8)
    else:
        ...
        p = sns.scatterplot(x=adata.obsm["X_umap"][:,0][:-n_proto-n_class*n_sample], y=adata.obsm["X_umap"][:,1][:-n_proto-n_class*n_sample], hue=[label_text[i] for i in label], s=s, palette=[pal[j] for j in range(len(label_text))], alpha=0.8)
    ...
    p.scatter(x=adata.obsm["X_umap"][-n_proto-n_class*n_sample:-n_class*n_sample,0], y=adata.obsm["X_umap"][-n_proto-n_class*n_sample:-n_class*n_sample,1], marker="p", s=250, color="black", label="P")
    ...
    for j in range(n_class):
        ...
        if i == n_class - 1:
            ...
            # myles
            p.scatter(x=adata.obsm["X_umap"][-(n_class-i)*n_sample:,0], y=adata.obsm["X_umap"][-(n_class-i)*n_sample:,1], marker="+", s=s_plus, color=pal_class[i], label=class_text[i], alpha=0.8)
        else:
            #original
            ...
            #myles
            p.scatter(x=adata.obsm["X_umap"][-(n_class-i)*n_sample:-(n_class-i-1)*n_sample,0], y=adata.obsm["X_umap"][-(n_class-i)*n_sample:-(n_class-i-1)*n_sample,1], marker="+", s=s_plus, color=pal_class[i], label=class_text[i], alpha=0.8)
