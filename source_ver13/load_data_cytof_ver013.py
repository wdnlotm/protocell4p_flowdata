...
def load_gausses(data_dir = "../data/gausses", load_ct=True, keep_sparse=False):
    
    #dat = sparse.load_npz(os.path.join(data_dir, "RawCounts.npz")) #no need. see adata_np_array
...
def load_cytof(data_dir = "../data", load_ct=True, keep_sparse=False, data_location = "../data"):
    # print(data_location)    #data_location = self.data_loc
    ...
    adata_np_array = np.genfromtxt(os.path.join(data_dir, "RawCounts.csv"), delimiter=',',dtype="float32") #done
    adata = sc.AnnData(adata_np_array, 
    ...
    for ind in tqdm.tqdm(sorted(set(meta.donor_id))):
        disease = list(set(meta.disease_ontology_label[meta.donor_id == ind]))
        assert len(disease) == 1
        if disease[0] == "long COVID-19" or disease[0] == "respiratory failure":
            continue
        x = adata.X[meta.donor_id == ind]
    ...
    if load_ct:
        ...
        return OurDataset(X=X, y=y, cell_id=barcodes, gene_id=genes, class_id=class_id) 
                          ...
                          #"../data/rh2_cf5_tcell_dim22_il1b_imp_v_noimp", load_ct=True, keep_sparse=False): 
def load_cytof_old(data_dir = "../data", load_ct=True, keep_sparse=False, data_location = "../data"):
    # print(data_location)    #data_location = self.data_loc
    print("loading data from" + data_dir)
    #dat = sparse.load_npz(os.path.join(data_dir, "RawCounts.npz")) #no need. see adata_np_array
