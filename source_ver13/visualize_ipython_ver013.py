import torch
from utils_ver013 import *
from config_cytof_ver013_alldata import *
...
def visualize_latent_sample(args, k=20, n_sample=None): # k: k cells that make most contribution for each sample's classification
    ...
    for i in idx:
        ...
        print("Feb 27 2024")
        print(args.checkpoint_name.replace(".pt", ""))
        print(X.shape[0])
        label = config.CT[config.SPLIT[i]:config.SPLIT[i+1]].cpu().numpy().tolist()
        # label_text = config.ct_id
        label_text = None
        model_name = args.checkpoint_name.replace(".pt", "")
        patid_text = find_patid(csv_file_name="patid_count.csv", cell_count=X.shape[0])
        file_end_txt = model_name+'_'+str(X.shape[0])+'_'+str(patid_text)+".png"
        print(file_end_txt)
        ############Commented by Myles. Uncomment if want plot umap
        # save_path = os.path.join(save_dir, str(i.item())+"_"+config.class_id[config.Y[i]]+file_end_txt)
