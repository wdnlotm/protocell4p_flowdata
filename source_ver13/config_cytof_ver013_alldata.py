...
class Config_eval:
    ...
    def __init__(self, data, data_loc, checkpoint_dir, checkpoint_name="best_model.pt", model="ProtoCell", \
        ...
        if data.lower() == "lupus":
            ...
            # dataset = load_cytof(load_ct=load_ct, keep_sparse=self.keep_sparse)      
        elif data.lower() == "cytof_reload":
            dataset = load_cytof_reload()
        else:
            raise ValueError("Data [:s] not supported".format(data))
    ...
    def reason(self, dataset="test"):
        if dataset == "train":
            ...
            print("Test data is hijacked")
            with open('RH2_selected_1034_1007_1072.pkl', 'rb') as file:   #Myles added
               inserted_data = pickle.load(file)                                 #Myles added
            loader = DataLoader(inserted_data, batch_size = self.batch_size, shuffle = False, collate_fn=self.collate_fn) #Myles added
        ...
        with torch.no_grad():
            for x, y, ct in tqdm.tqdm(loader):
                ...
                print("Config_eval saving import_scores, c_logits, logits")  #Myles added
                ...
                print("Printing c logit shape and saving it")
                print(self.checkpoint_name)
                print(c_logits.shape[0])
                
                model_num = (self.checkpoint_name).replace(".pt","")
                cell_count_num = c_logits.shape[0]
                patid_num = find_patid(csv_file_name="patid_count.csv", cell_count=c_logits.shape[0])
                file_name0 = f'c_logits_{patid_num}_{model_num}_{cell_count_num}.pkl'
                # print(file_name0)
                
                # file_name1 = 'c_logits_' + (self.checkpoint_name).replace(".pt","") + '_' + str(c_logits.shape[0])
                # print(f'file_name1: {file_name1}')
                # patid_text = find_patid(csv_file_name="patid_count.csv", cell_count=c_logits.shape[0])
                # print(f'patid_text: {patid_text}')
                
                # file_name = file_name1 +'_'+ str(patid_text) +'.pkl'  #'c_logits_all_data.pkl'
                with open(file_name0, 'wb') as file:
...
def patid_identify(csv_file_name, train_data, val_data , test_data ):
    ...
    for ii in range(len(train_data)):
        ...
        pvo2 = (cell_count['peakvo2'][cell_count['count']==c_count]).iloc[0]
